...
func (c *CliWekaService) UpdateSmbwCluster(ctx context.Context, params SmbwUpdateParams) error {
	...
	// Add cluster name if not default
	if params.ClusterName != "" && params.ClusterName != "default" {
		cmd = append(cmd, "--cluster-name", params.ClusterName)
	}
...
func (c *CliWekaService) CreateFilesystem(ctx context.Context, name, group string, params FSParams) error {
	...
	if params.IsEncrypted {
		...
		if params.NoKmsEncryption {
			cmd = append(cmd, "--allow-no-kms")
		}
	}
